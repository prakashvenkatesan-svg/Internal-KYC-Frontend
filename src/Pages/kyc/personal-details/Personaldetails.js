...
const PersonalDetails = () => {
  ...
  useEffect(() => {
    ...
    // restore saved DDPI if available
    const savedDdpi = localStorage.getItem("ddpi");
    if (savedDdpi) {
      setFormData((prev) => ({
        ...prev,
        ddpi: savedDdpi,
  ...
  useEffect(() => {
    ...
    const loadAadhaarAddressPrefill = async () => {
      try {
        ...
        if (!fetchedAddress || ignore) {
          return;
        }

        setFormData((prev) => {
          if (String(prev.aadhaarAddress || "").trim()) {
            return prev;
          }

          return {
            ...prev,
            aadhaarAddress: fetchedAddress,
          };
        });
      } catch (error) {
        console.log(
          "Unable to prefill Aadhaar address:",
      ...
      <div className='personal-card'>
        <form onSubmit={handleSubmit}>
              ...
              <div className='row g-3'>
                ...
                <div className='col-12'>
                  ...
                  <p className='mt-2' style={{ color: "#264095" }}>
                    If you enable DDPI, one stamp paper will be assigned
                    automatically and shown again during document review before
                    eSign.
                  </p>
                </div>
              </div>
          ...
          <div className='d-flex gap-3'>
            <input
              ...
              onChange={handleChange}
            />
            <label htmlFor='rightsObligations'>
        ...
        {showDdpiPopup && (
          <div className='modal-overlay'>
            <div className='ddpi-modal-box'>
              <div className='ddpi-modal-header'>
                <h3>
                  Do you wish to execute DDPI (Demat Debit Pledge Instructions)
                  <span>*</span>
                </h3>
              </div>

              <div className='ddpi-modal-body'>
                <p className='ddpi-text'>You have selected the DDPI Option</p>
                <h1 className='ddpi-amount'>₹ 150</h1>
                <p className='ddpi-subtext'>(Additional Charges)</p>
              </div>

              <div className='ddpi-modal-footer'>
                <button
                  type='button'
                  className='ddpi-no-btn'
                  onClick={handleDdpiNo}
                >
                  No
                </button>
